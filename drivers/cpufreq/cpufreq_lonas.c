...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_LONAS
static
#endif
struct cpufreq_governor cpufreq_gov_lonas = {
	.name                   = "lonas",
...
struct workqueue_struct *dvfs_lonas_workqueue;
...
static struct dbs_tuners {
	...
	unsigned int cpu_max_freq_current; /*  Current CPU freq. limit */
	unsigned int cpu_max_freq_1;       /*  CPU freq. limit running on 1 core */
	unsigned int cpu_max_freq_2;       /*  CPU freq. limit running on 2 cores */
	unsigned int cpu_max_freq_3;       /*  CPU freq. limit running on 3 cores */
	unsigned int cpu_max_freq_4;       /*  CPU freq. limit running on 4 cores */
	...
	.cpu_max_freq_current = DEF_CPU_MAX_FREQ_4, /*  Current CPU freq. limit set at 4 cores level */
	.cpu_max_freq_1 = DEF_CPU_MAX_FREQ_1,       /*  CPU freq. limit running on 1 core */
	.cpu_max_freq_2 = DEF_CPU_MAX_FREQ_2,       /*  CPU freq. limit running on 2 cores */
	.cpu_max_freq_3 = DEF_CPU_MAX_FREQ_3,       /*  CPU freq. limit running on 3 cores */
	.cpu_max_freq_4 = DEF_CPU_MAX_FREQ_4,       /*  CPU freq. limit running on 4 cores */
...
static void apply_hotplug_lock(void)
{
	...
	queue_work_on(dbs_info->cpu, dvfs_lonas_workqueue, work);
}

int cpufreq_lonas_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_lonas_cpu_unlock(int num_core)
{
...
void cpufreq_lonas_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpu_dbs_info_s *dbs_info;
	...
	if (flag <= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_lonas_workqueue, &dbs_info->up_work);
}

void cpufreq_lonas_min_cpu_unlock(void)
{
	...
	if (flag >= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_lonas_workqueue, &dbs_info->down_work);
}
...
struct cpu_usage_history *hotplug_lonas_history;
...
/* cpufreq_lonas Governor Tunables */
...
show_one(cpu_max_freq_current, cpu_max_freq_current); /*  Current CPU freq. limit */
show_one(cpu_max_freq_1, cpu_max_freq_1);             /*  CPU freq. limit running on 1 core */
show_one(cpu_max_freq_2, cpu_max_freq_2);             /*  CPU freq. limit running on 2 cores */
show_one(cpu_max_freq_3, cpu_max_freq_3);             /*  CPU freq. limit running on 3 cores */
show_one(cpu_max_freq_4, cpu_max_freq_4);             /*  CPU freq. limit running on 4 cores */
...
/*  CPU freq. limit running on 1 core */
...
/*  CPU freq. limit running on 2 cores */
...
/*  CPU freq. limit running on 3 cores */
...
/*  CPU freq. limit running on 4 cores */
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_lonas_min_cpu_unlock();
	else
		cpufreq_lonas_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_lonas_cpu_unlock(prev_lock);
	...
	ret = cpufreq_lonas_cpu_lock(input);
	if (ret) {
...
define_one_global_rw(cpu_max_freq_current); /*  Current CPU freq. limit (should be ro, but nevermind for now) */
define_one_global_rw(cpu_max_freq_1); /*  CPU freq. limit running on 1 core */
define_one_global_rw(cpu_max_freq_2); /*  CPU freq. limit running on 2 core */
define_one_global_rw(cpu_max_freq_3); /*  CPU freq. limit running on 3 core */
define_one_global_rw(cpu_max_freq_4); /*  CPU freq. limit running on 4 core */
...
static struct attribute *dbs_attributes[] = {
	...
	&cpu_up_freq.attr,
	&cpu_down_freq.attr,
	&cpu_max_freq_current.attr, /*  Current CPU freq. limit */
	&cpu_max_freq_1.attr, /*  CPU freq. limit running on 1 core */
	&cpu_max_freq_2.attr, /*  CPU freq. limit running on 2 cores */
	&cpu_max_freq_3.attr, /*  CPU freq. limit running on 3 cores */
	&cpu_max_freq_4.attr, /*  CPU freq. limit running on 4 cores */
	&up_nr_cpus.attr,
...
static void cpu_up_work(struct work_struct *work)
{
	...
	/*  Store current desired CPU freq. limit based on number online cores */
...
{
	int num_hist = hotplug_lonas_history->num_hist;
	struct cpu_usage *usage;
	int freq, rq_avg;
	int i;
	...
	for (i = num_hist - 1; i >= num_hist - up_rate; --i) {
		usage = &hotplug_lonas_history->usage[i];
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load_freq > up_threshold * policy->cur) {
		...
		/*  Keep CPU freq. to the current CPU freq. max */
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, dvfs_lonas_workqueue, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, dvfs_lonas_workqueue,
...
unsigned int lonas_prev_freq_step;
unsigned int lonas_prev_sampling_rate;
static void cpufreq_lonas_early_suspend(struct early_suspend *h)
{
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	hotplug_lonas_history = kzalloc(sizeof(struct cpu_usage_history), GFP_KERNEL);
	if (!hotplug_lonas_history) {
		pr_err("%s cannot create hotplug history array\n", __func__);
		ret = -ENOMEM;
		goto err_hist;
	}

	dvfs_lonas_workqueue = create_workqueue("lonas");
	if (!dvfs_lonas_workqueue) {
		pr_err("%s cannot create workqueue\n", __func__);
		ret = -ENOMEM;
		goto err_queue;
	}

	ret = cpufreq_register_governor(&cpufreq_gov_lonas);
	if (ret)
		goto err_reg;

#ifdef CONFIG_HAS_EARLYSUSPEND
	early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	early_suspend.suspend = cpufreq_lonas_early_suspend;
	early_suspend.resume = cpufreq_lonas_late_resume;
#endif

	return ret;

err_reg:
	destroy_workqueue(dvfs_lonas_workqueue);
err_queue:
	kfree(hotplug_lonas_history);
err_hist:
	return ret;
}
...
{
	cpufreq_unregister_governor(&cpufreq_gov_lonas);
	destroy_workqueue(dvfs_lonas_workqueue);
	kfree(hotplug_lonas_history);
}

MODULE_AUTHOR("ByungChang Cha <bc.cha@samsung.com>");
MODULE_DESCRIPTION("'cpufreq_lonas' - A dynamic cpufreq/cpuhotplug governor");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_LONAS
